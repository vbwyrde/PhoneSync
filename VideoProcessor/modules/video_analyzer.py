...
"""
    ...
    def get_analysis_statistics(self) -> Dict[str, Union[int, float, bool]]:
        """Get video analysis statistics"""
        # Create a new dictionary with proper types to avoid type conflicts
        stats: Dict[str, Union[int, float, bool]] = {
            ...
            'analysis_failures': self.stats['analysis_failures'],
            'thumbnails_extracted': self.stats['thumbnails_extracted']
        }

        # Add calculated fields as floats
        if stats['videos_analyzed'] > 0:
            stats['kung_fu_detection_rate'] = round((stats['kung_fu_detected'] / stats['videos_analyzed']) * 100, 2)
            stats['success_rate'] = round(((stats['videos_analyzed']) /
                                   (stats['videos_analyzed'] + stats['analysis_failures'])) * 100, 2)
        else:
            stats['kung_fu_detection_rate'] = 0.0
